...
void dphi_mc_data_compr_one_mode(bool substract_plateau){
  ...
  for (unsigned int ksign = 0; ksign < nSigns; ksign++){
    ...
    h[0][ksign]->SetMarkerStyle(20);
    h[0][ksign]->SetMarkerSize(0.6);

    hist_helper(h[2][ksign], norm_factor[2], "#frac{d#sigma}{d #Delta #phi} [pb]");
    l->AddEntry(h[2][ksign],dtTitles[2].c_str(),"lp");
    h[2][ksign]->SetMarkerStyle(20);
    h[2][ksign]->SetMarkerSize(0.6);

    std::cout << h[0][ksign]->Integral("width") << std::endl;

    // std::cout << h[0][ksign]->Integral("width") << ", " << h[2][ksign]->Integral("width") * 256.8 << std::endl;
                
    h[0][ksign]->SetMarkerColor(kRed);
    h[0][ksign]->SetLineColor(kRed);
    h[2][ksign]->SetMarkerColor(kBlack);
    h[2][ksign]->SetMarkerStyle(20);
    h[2][ksign]->SetLineColor(kBlack);
