...
std::string png_title_1D[nCtrBins][nGapCuts];
std::string png_title_2D[nGroups][nCtrBins][nGapCuts];
std::string subplot_titles[ndRs][nSigns];
std::string subplot_dphi_titles[ndRs][nSigns];
// std::string subplot_titles_1D[ndRs][nSigns];
// std::string subplot_titles_2D[ndRs][nSigns][nGroups];
...
void initialize(){
	...
	for (unsigned int ksign = 0; ksign < nSigns; ksign++){
		...
		for (unsigned int jdphi = 0; jdphi < ndRs; jdphi++){  	
	...
	// for (unsigned int ictr = 0; ictr < 1; ictr++){
		png_title_1D[ictr][0] = "_real_scr_ctr" + std::to_string(ictr+1) + "_no_gap_cut.png";
		png_title_1D[ictr][1] = "_real_scr_ctr" + std::to_string(ictr+1) + "_with_gap_cut.png";
		png_title_2D[0][ictr][0] = "_real_ctr" + std::to_string(ictr+1) + "_no_gap_cut.png";
		png_title_2D[0][ictr][1] = "_real_ctr" + std::to_string(ictr+1) + "_with_gap_cut.png";
		png_title_2D[1][ictr][0] = "_scr_ctr" + std::to_string(ictr+1) + "_no_gap_cut.png";
		png_title_2D[1][ictr][1] = "_scr_ctr" + std::to_string(ictr+1) + "_with_gap_cut.png";
	}
}
...
void plot1D(Hist1D& h, const int nCuts, std::string cuts[], std::string cutTitles[], std::string name_specf=""){
	...
	for (int ictr = 0; ictr < nCtrBins; ictr++){
		for (int mgapcut = 0; mgapcut < nGapCuts; mgapcut++){
			...
			char name[100];
			sprintf(name, "c_%s_ctr%d_gapcut%d", h.name.c_str(), ictr, mgapcut);
			c[ictr] = new TCanvas(name,name,4200,2000);
			c[ictr]->Divide(nCuts,nSigns);
			...
			c[ictr]->SaveAs(("plots/real_scramb_comparison/" + h.name + h.name_specifier + png_title_1D[ictr][mgapcut]).c_str());
	  		c[ictr]->Close();
	  		delete c[ictr];
...
// void plot1D(TH1D* h[nCtrBins][nGapCuts][ndRs][nSigns][nGroups], std::string hist1DName, bool logx, bool logy, std::string name_specifier = ""){
void plot1D(TH1D (*h)[nCtrBins][nGapCuts][nSigns][nGroups], Hist1D& h_struct, const int nCuts, std::string cuts[], std::string cutTitles[]){
	TCanvas *c[nCtrBins];
	...
	for (int ictr = 0; ictr < nCtrBins; ictr++){
		for (int mgapcut = 0; mgapcut < nGapCuts; mgapcut++){
			...
			char name[100];
			sprintf(name, "c_%s_ctr%d_gapcut%d", h_struct.name.c_str(), ictr, mgapcut);
			c[ictr] = new TCanvas(name,name,4200,2000);
			c[ictr]->Divide(nCuts,nSigns);
			...
			for (unsigned int ksign = 0; ksign < nSigns; ksign++){
				for (unsigned int jcut = 0; jcut < nCuts; jcut++){
	    			...
	    			TH1D* h1[nGroups] = {&h[jcut][ictr][mgapcut][ksign][0], &h[jcut][ictr][mgapcut][ksign][1]};
					for (int igrp = 0; igrp < nGroups; igrp++){
						std::string subplot_title = cutTitles[jcut] + ", " + signTitles[ksign];
						hist_helper(h1[igrp],subplot_title, true);
	    				l->AddEntry(h1[igrp],grpTitles[igrp].c_str(),"lp");
			...
			c[ictr]->SaveAs(("plots/real_scramb_comparison/" + h_struct.name + h_struct.name_specifier + png_title_1D[ictr][mgapcut]).c_str());
	  		c[ictr]->Close();
	  		delete c[ictr];
...
void Process2D(Hist2D h, const int nCuts, std::string cuts[], std::string cutTitles[], std::string name_specf=""){
	...
	h.name_specifier = name_specf;
	// TH1D (*hx)[nGapCuts][nCuts][nSigns][nGroups] = new TH1D[nCtrBins][nGapCuts][nCuts][nSigns][nGroups];
	// TH1D (*hy)[nGapCuts][nCuts][nSigns][nGroups] = new TH1D[nCtrBins][nGapCuts][nCuts][nSigns][nGroups];
	TH1D (*hx)[nCtrBins][nGapCuts][nSigns][nGroups] = new TH1D[nCuts][nCtrBins][nGapCuts][nSigns][nGroups];
	TH1D (*hy)[nCtrBins][nGapCuts][nSigns][nGroups] = new TH1D[nCuts][nCtrBins][nGapCuts][nSigns][nGroups];

	TCanvas *c[nCtrBins][nGapCuts][nGroups];

	for (int ictr = 0; ictr < nCtrBins; ictr++){
		for (unsigned int mgapcut = 0; mgapcut < nGapCuts; mgapcut++){
			for (int lgrp = 0; lgrp < nGroups; lgrp++){
				char name[100];
				sprintf(name, "c_%s_ctr%d_gapcut%d_group%d", h.name.c_str(), ictr, mgapcut,lgrp);
				c[ictr][mgapcut][lgrp] = new TCanvas(name,name,4200,2000);
				c[ictr][mgapcut][lgrp]->Divide(nCuts,nSigns);
				...
				for (unsigned int ksign = 0; ksign < nSigns; ksign++){
					for (unsigned int jcut = 0; jcut < nCuts; jcut++){
						...
						c[ictr][mgapcut][lgrp]->cd(ksign * nCuts + jcut + 1);
						gPad->SetLogx(h.logx);
						gPad->SetLogy(h.logy);
						gPad->SetLeftMargin(0.16);
						gPad->SetBottomMargin(0.135);
						...
						hx[jcut][ictr][mgapcut][ksign][lgrp] = *(h2->ProjectionX());
						hy[jcut][ictr][mgapcut][ksign][lgrp] = *(h2->ProjectionY());
				...
				c[ictr][mgapcut][lgrp]->SaveAs(("plots/real_scramb_comparison/" + h.name + h.name_specifier + png_title_2D[lgrp][ictr][mgapcut]).c_str());
  				c[ictr][mgapcut][lgrp]->Close();
  				delete c[ictr][mgapcut][lgrp];
...
void plot_real_scramb_comparison(){
