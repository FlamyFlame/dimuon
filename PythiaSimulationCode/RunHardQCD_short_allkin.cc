...
int main(int argc, char **argv) 
{
  ...
  int seed = 600000 + 10000 * atoi(argv[2]) + 1000 * atoi(argv[3]) + atoi(argv[1]);
  ...
  std::vector<double>* b_px=new std::vector<double>(); // Kinematics
  std::vector<double>* b_py=new std::vector<double>(); // Kinematics
  std::vector<double>* b_pz=new std::vector<double>(); // Kinematics
  std::vector<double>* b_pAbs=new std::vector<double>(); // Kinematics
  std::vector<double>* b_e=new std::vector<double>(); // Kinematics
  std::vector<double>* b_y=new std::vector<double>(); // Kinematics
  std::vector<double>* b_scale=new std::vector<double>(); // Kinematics
  std::vector<double>* b_m2=new std::vector<double>(); // Kinematics
  std::vector<double>* b_mCalc=new std::vector<double>(); // Kinematics
  std::vector<double>* b_mCalcc=new std::vector<double>(); // Kinematics
  ...
  T->Branch("truth_px",&b_px);
  T->Branch("truth_py",&b_py);
  T->Branch("truth_pz",&b_pz);
  T->Branch("truth_pAbs",&b_pAbs);
  T->Branch("truth_e",&b_e);
  T->Branch("truth_y",&b_y);
  T->Branch("truth_scale",&b_scale);
  T->Branch("truth_m2",&b_m2);
  T->Branch("truth_mCalc",&b_mCalc);
  T->Branch("truth_mCalcc",&b_mCalcc);
  ...
  for (int iEvent = 0; iEvent < nEvent;){
    ...
    b_px->reserve(event.size());
    b_py->reserve(event.size());
    b_pz->reserve(event.size());
    b_pAbs->reserve(event.size());
    b_e->reserve(event.size());
    b_y->reserve(event.size());
    b_scale->reserve(event.size());
    b_m2->reserve(event.size());
    b_mCalc->reserve(event.size());
    b_mCalcc->reserve(event.size());
    ...
    if (b_nmuons >= 2){
      ...
      for(int ip=0; ip<event.size(); ip++){// Loop through all of the output particles in the event
        ...
        b_py->push_back(p.py());
        b_pz->push_back(p.pz());
        b_pAbs->push_back(p.pAbs());
        b_e->push_back(p.e());
        b_y->push_back(p.y());
        b_scale->push_back(p.scale());
        b_m2->push_back(p.m2());
        b_mCalc->push_back(p.mCalc());
        double msq = pow(p.e(),2) - p.pAbs2();
        b_mCalcc->push_back((msq >= 0)? sqrt(msq) : -sqrt(-msq));
    	}
