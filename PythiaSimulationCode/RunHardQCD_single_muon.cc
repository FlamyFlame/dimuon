...
int main(int argc, char **argv) 
{
  ...
  FilterMode filter_mode = FilterMode::single_muon;
  ...
  if (kinematicRange == 0) nEvent = 100; // 5-8
  if (kinematicRange == 1 || kinematicRange == 2) nEvent = 100; // 8-14, 14-24
  else if (kinematicRange == 3) nEvent = 1000; // 24-40
  else if (kinematicRange == 4) nEvent = 50000; // 40-70
  else nEvent = 200000; // 70-125, 125-300
  ...
  for (int iEvent = 0; iEvent < nEvent;){
    ...
    if (b_nmuons >= nmuons_required){
      ...
      if (kinematicRange == 0 && iEvent != 0){
        ...
        int nstep = -1;
        if (kinematicRange == 0 || kinematicRange == 1 || kinematicRange == 2 || kinematicRange == 3)      nstep = 100;
        else if (kinematicRange == 4) nstep = 1000;
        else                          nstep = 2000;
