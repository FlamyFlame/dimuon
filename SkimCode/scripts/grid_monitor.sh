...
apply_mode_config() {
	case "$MODE" in
		...
		fullsim_pp)
			...
			;;
		fullsim_pp_full)
			# pp24-fullsim FULL sample (_pdf production, DSIDs 803015-803020).
			# Same NTUP basenames as the test sample — different directory.
			DATA_BASE="/usatlas/u/yuhanguo/usatlasdata/pythia_fullsim_full_sample"
			RECORD_FILE="${DATA_BASE}/merging-record.txt"
			IS_MC_FLAT=1
			;;
		*)
...
if [[ ${#TASK_IDS[@]} -eq 0 ]]; then
	echo "ERROR: no task IDs provided. Usage: $0 [--mode data|overlay|fullsim_pp|fullsim_pp_full] [-i POLL_MINUTES] FILE_OR_TASKIDS..." >&2
	exit 1
fi
