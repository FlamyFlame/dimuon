...
namespace Trig {
  ...
  Bool_t match(const xAOD::Muon* mu,
  ...
  Bool_t matchL1(const xAOD::Muon* mu,
  ...
  Double_t minDelR(const xAOD::Muon* mu,
				     const std::string &chain,
				     const double mindelR) const
  {
  ...
  Bool_t matchL1(const double eta,
				   const double phi,
				   const std::string &l1item,
				   const double DelR) const
  {
    ...
    if(!sc){
      ATH_MSG_ERROR("matchL1 : could not retrieve LVL1MuonRoIs");
      return false;
    }
	...
	Double_t dR = dR(eta, phi, (*muroi_itr)->eta(), (*muroi_itr)->phi());
	if(dR < l1dr){
	  return true;
	}
      }
  ...
  Bool_t matchL2SA(const xAOD::Muon* mu,
				     const std::string &l1item,
				     const std::string &chain,
				     const double DelR) const
  {
	...
	for(auto muon : *mucont.cptr()){
	  if(muon->roiNumber() == ROI){
	    Double_t dR = dR(mu->eta(), mu->phi(), muon->eta(), muon->phi());
  ...
  Bool_t matchL2CB(const xAOD::Muon* mu,
				     const std::string &chain,
				     const double DelR) const
  {
    const std::string eventTrigger = chain;
  ...
  Bool_t matchDimuon(const TLorentzVector& muon1,
  ...
  Bool_t isPassedRerun(const std::string& trigger) const
  {
  ...
  std::pair<Bool_t, Bool_t> matchDimuon(const TLorentzVector& muon1,
							  const TLorentzVector& muon2,
  ...
  int getL1pt(const std::string& l1item) const
  {
    ...
    if (tokens.size() == 1) {
      ...
      ATH_MSG_ERROR("getL1pt : cannot parse " << l1item);
      return rc;
    }
