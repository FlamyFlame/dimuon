#ifndef __MuonPPNTupleFirstPass_h__
#define __MuonPPNTupleFirstPass_h__
...
public :
    MuonPPNTupleFirstPass();
    ~MuonPPNTupleFirstPass(){}
    void Run();
};


MuonPPNTupleFirstPass::MuonPPNTupleFirstPass(){
    if(mode != 3 && mode != 4){
...
void MuonPPNTupleFirstPass::InitInput(){

    fChain = new TChain("HeavyIonD3PD","HeavyIonD3PD");
    fChain->SetMakeClass(1);
    fChain->Add("New_All_DataPP2017_5TeV_Dec2021.root");
...
void MuonPPNTupleFirstPass::InitOutput(){

    if (mode == 3){
        m_outfile=new TFile("single_muon_trees_pp.root","recreate");
        muonOutTree = new TTree("muon_tree","all single muons");
        muonOutTree->Branch("MuonObj",&tempmuon);
    }
    else{ //output muon pair trees
        m_outfile=new TFile("muon_pairs_pp.root","recreate");
