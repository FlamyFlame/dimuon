---
description: Executor-reviewer loop for writing or editing internal note sections
---
...
- `.claude/agents/note-reviewer.md` (document consistency — internal note checklist items 1-8)
...
Configuration:
- MAX_ITERATIONS = 5
- LOG_FILE = `.claude/logs/review-note-$(date +%Y%m%d-%H%M%S).md`
...
#### Execute
Write or edit the internal note section described in $ARGUMENTS. Follow the existing note structure in `IntNote/`. Match the LaTeX style and conventions already used in `IntNote/tex/`.

#### Review
Switch to reviewer mode. Apply ALL three review checklists:

**Note review** (from `note-reviewer.md`, items 1-8): figure references, number consistency, acronyms, citations, captions, placeholders, LaTeX compilation.

**Physics review** (from `physics-reviewer.md`): are physics statements correct? Are conventions used consistently?
