...
1. **Get the PDF.** PRIMARY → `curl -sSL <url> -o <area-dir>/<file>.pdf` (commit
   it). SUPPORTIVE → record the URL; `curl` to `/tmp` only if you need to read it.
2. **Read it with ghostscript** (`gs -sDEVICE=txtwrite`, page ranges). Never use
   WebFetch or the Read tool on the PDF.
...
- Write **only your one source doc** (and, if PRIMARY, commit its PDF).
