...
# make_JOs.sh  ─ generate 24 Pythia8 job‑option files from mc.template.5.36TeV.py
template="mc.template.5.36TeV.py"              # source JO (pp, 60‑120 slice)
...
for i in "${!isospins[@]}"; do
  ...
  for j in "${!labels[@]}"; do
    ...
    out="JOs_5.36TeV/mc.Py8EG_A14_5360_${iso}_hQCD_DiMu_pTH${tag}.py"
