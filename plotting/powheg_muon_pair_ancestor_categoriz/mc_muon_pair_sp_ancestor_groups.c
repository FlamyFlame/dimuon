...
void plot_one_mode(int mode){ // mode = 1: unweighted; mode = 2: weighted
  ...
  if (mode == 1){
    ...
    hist_names.clear();
    hist_names.push_back({{"h_QQ_both_from_Q_ancestor_sp_sign1_near","h_QQ_both_from_Q_ancestor_sp_sign1_away"},{"h_QQ_both_from_Q_ancestor_sp_sign2_near","h_QQ_both_from_Q_ancestor_sp_sign2_away"}});
    hist_names.push_back({{"h_QQ_both_from_Q_ancestor_sp_sign1_near","h_QQ_both_from_Q_ancestor_sp_sign1_away"},{"h_QQ_both_from_Q_ancestor_sp_sign2_near","h_QQ_both_from_Q_ancestor_sp_sign2_away"}});
  }
  ...
  for (int imc = 0; imc < nMCmodes; imc++){
    ...
    if (mode == 1) c->SaveAs(Form("plots/powheg/prt_grouping/muon_pair_sp_ancestor_groups_%s.png",mcmodes[imc].c_str()));
    else           c->SaveAs(Form("plots/powheg/prt_grouping/muon_pair_sp_ancestor_groups_%s_weighted.png",mcmodes[imc].c_str()));
    c->Close();
    delete c;
  }
