...
void plot_hard_scatt_single_kinematic(std::string kin, bool projx_2d, bool projy_2d, bool add, bool norm_unity, std::string kin1d, bool logx=false){
  ...
  for (int imc = 0; imc < nMCmodes; imc++){
    ...
    if (add){
      c->SaveAs(Form("plots/mc_truth/accumulative/hard_scatt/hard_scatt_near_away_%s%s_accumulative.png", kin1d.c_str(), mcmodes[imc].c_str()));
    }else if (norm_unity){
      c->SaveAs(Form("plots/mc_truth/hard_scatt/hard_scatt_near_away_%s%s_unity.png", kin1d.c_str(), mcmodes[imc].c_str()));
    }else{
      c->SaveAs(Form("plots/mc_truth/hard_scatt/hard_scatt_near_away_%s%s.png", kin1d.c_str(), mcmodes[imc].c_str()));
    }
    c->Close();
    delete c;
...
void plot_hard_scatt_near_away(){
