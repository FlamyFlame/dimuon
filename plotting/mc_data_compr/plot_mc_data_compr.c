...
TFile* f[nDtTypes];
std::string dt_paths[nDtTypes] = {"/usatlas/u/yuhanguo/usatlasdata/athena/runMCV2/","/usatlas/u/yuhanguo/usatlasdata/athena/runMCV2/","/usatlas/u/yuhanguo/usatlasdata/pythia/","/usatlas/u/yuhanguo/usatlasdata/dimuon_data/"};
std::string fnames[nDtTypes] = {"histograms_mc_truth_bb.root","histograms_mc_truth_cc.root","histograms_pythia.root","histograms_real_pairs_pp.root"};
...
void plot_mc_data_compr_single_kinematic(std::string kin, bool projx_2d, bool projy_2d, bool norm_unity, std::string kin1d, std::string kin_title, bool logx=false){
  ...
  for (unsigned int ksign = 0; ksign < nSigns; ksign++){
    for (unsigned int jdphi = 0; jdphi < 2; jdphi++){
      ...
      for (unsigned int idt = 0; idt < nDtTypes; idt++){
        std::string hist_gapcut_postfix = (idt == 2)? "" : "_gapcut1";
  ...
  if (norm_unity){
    c->SaveAs(Form("plots/mc_data_compr/%s_mc_data_compr_unity.png", kin1d.c_str()));
  }else{
    c->SaveAs(Form("plots/mc_data_compr/%s_mc_data_compr.png", kin1d.c_str()));
  }
