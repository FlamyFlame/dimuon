...
const int nDtTypes = 2;

double pi = acos(-1.0);
float norm_factor[nDtTypes] = {1/256.8, 1/256.8}; // normalizing to differential crossx; unit is pb
Color_t colors[nDtTypes] = {kBlack, kRed};


TFile* f[nDtTypes];
std::string dt_path = "/usatlas/u/yuhanguo/usatlasdata/dimuon_data/";
std::string fnames[nDtTypes] = {"histograms_real_pairs_pp_no_resn_no_photo.root","histograms_real_pairs_pp_no_resn_cut.root"};
std::string dtTitles[nDtTypes] = {"pp - no photoproduction/resosnance cut", "pp - with photoproduction cut"};
...
void dphi_photo_cut_one_mode(bool ratio){
  ...
  if (ratio)  c->SaveAs("plots/pp_data/resn_cuts/dphi_photo_cut_ratio.png");
  else        c->SaveAs("plots/pp_data/resn_cuts/dphi_photo_cut.png");
  c->Close();
  delete c;
...
void dphi_photo_cut(){
  dphi_photo_cut_one_mode(false);
  dphi_photo_cut_one_mode(true);
}
