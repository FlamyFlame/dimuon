...
void plot_mc_data_compr_single_kinematic(std::string kin, bool projx_2d, bool projy_2d, bool norm_unity, std::string kin1d, std::string kin_title, bool logx=false){
  ...
  for (unsigned int ksign = 0; ksign < nSigns; ksign++){
    ...
    TLegend* l = new TLegend(0.55,0.72,0.87,0.87);
    ...
    for (unsigned int jdt = 0; jdt < nDtTypes; jdt++){
      if (projx_2d){
        ...
        if (norm_unity){
          h[jdt][ksign] = (TH1D*) h2d->ProjectionX(Form("h_unity_%d%d",jdt+1,ksign+1));
        }else{
          h[jdt][ksign] = (TH1D*) h2d->ProjectionX(Form("h_%d%d",jdt+1,ksign+1));
        }
      }else if (projy_2d){
        TH2D* h2d = (TH2D*) f[jdt]->Get(("h_" + kin + "_dr3" + signs[ksign] + "_gapcut1").c_str());
        if (norm_unity){
          h[jdt][ksign] = (TH1D*) h2d->ProjectionY(Form("h_unity_%d%d",jdt+1,ksign+1));
        }else{
          h[jdt][ksign] = (TH1D*) h2d->ProjectionY(Form("h_%d%d",jdt+1,ksign+1));
        }
      }else{ // 1D
    ...
    hist_helper(h[0][ksign], norm_factor[0], Form("#frac{d#sigma}{d %s} [pb]", kin_title.c_str()));
