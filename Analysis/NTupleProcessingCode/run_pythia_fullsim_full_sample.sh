...
#   2. expected_ami_dsids -> the "_pdf" evgen's DSIDs. The AMI directory itself is NOT set here:
#      isTestSample=false + FullSimSampleType::pp => PythiaEvgen::PDF => the proton-PDF evgen's
#      ami_info_PDF/ (FullSimSampleType.h; docs/ami_weights.md). AMI files are keyed by
#      beam+slice ONLY, so the nPDF evgen's files would open just as well and weight the sample
#      with the TEST sample's sigma*eff (pTH8_14: 23.78 nb vs the correct 36.74 nb -- a 55% error
#      that VARIES BY SLICE and therefore does NOT cancel in ratios). expected_ami_dsids makes
#      InitInputFullsim THROW if the wrong evgen is picked up.
