...
| 2026-06-16 | **task_06 R_AA DONE** (reco-corrected R_AA runs) | Added SS signal-region histos to RDF crossx (PbPb 3D `h3d_ss_..._vs_centr`, pp 2D `h2d_ss_...`) for OS−SS combinatorial subtraction (`/review-analysis-code` PASS). Modernized `RAA_plotting.cxx` (cluster paths, RDF inputs case 6, combined PbPb 23+24+25 vs pp24, OS−SS, 15-bin pT, index-wise ratio, segfault+mode-3 off-by-one fixed; `/review-analysis-code` PASS iter2). R_AA plots vs pair pT/η/centrality (`/review-plot` PASS iter4). 2025 lumi verified 2.59933 nb⁻¹; 2023 T_AA placeholder. Tracking: `raa_from_rdf_crossx.md`. **OPEN: year-combination normalization** (naive sum of 1/L_year-weighted histos, shared w/ crossx, inflates abs scale ~Σ(years)) → needs consistent fix across crossx + R_AA |
