...
| 9 | Trigger efficiency: dR correlation corrections | DRAFT+PH → **UNBLOCKED 2026-07-10** | Method settled (D9: cross-term as MC reference). The unbiased-MC prerequisite is **met**: Run-3 fullsim/overlay AODs carry trigger sim and the `_July2026` re-skim stores the decisions + per-leg 2mu4 matching with all events kept (`mc_trigger_info_skim.md`, CLOSED). Numbers to be produced in `mc_trigger_efficiency.md`; carry the ~20% MC over-efficiency of the **second** muon leg as a systematic |
...
| 16 | Background subtraction (full program): (a) Δp/p fake-muon purity **[π/K background MC EXISTS — see note; sequencing vs RAA TBD after the V2 background-MC investigation]**, (b) correlated-physics (g→QQ̄ / FE) **minv template fit** — NOW **OS−SS + MC(S+G)** (combined OS+SS/k-anchored fit ABANDONED 2026-06-24, root cause = soft near-side combinatoric; see `low_mass_dimuon_template_fit.md`), (c) signal acceptance | IN PROGRESS (autonomous impl.) | **CORRECTION 2026-06-24: background MC is NOT missing.** The fullsim/overlay reco muon collection is skimmed like data → contains fake (`muon_truth_prob≤0.5`) + hadronic (`|muon_truth_id|≠13` punch-through, or `id=13 & IsPrimary=0` decay-in-flight) muons; classifiable per reco muon. Pythia Run3 pp fullsim + POWHEG Run2 pp17 fullsim (dcache) usable for the **physics test** (not detector-condition procedures). This enables BOTH the V2 low-mass background demonstration AND the Δp/p fake-template fit (reco quantities, before unfolding, nominal pipeline). The minv template-fit program is tracked in `low_mass_dimuon_template_fit.md`. **NOTE: the current crossx/R_AA `OS − SS` subtraction (steps 14/15) is PROVISIONAL** — removes only combinatoric, leaving `(1−k)·G`; replaced by this program |
...
| dR trigger-correlation correction (final) | ~~Unbiased trigger decision in MC → needs fullsim overlay with trigger sim~~ **PREREQUISITE SATISFIED 2026-07-10** — the Run-3 fullsim AODs (r16578 pp; r17618/r17662 overlay) all ran trigger simulation, and the `_July2026` re-skim carries the trigger decisions + per-leg 2mu4 matching branches, with **all events kept** (no trigger-OR filter). See `mc_trigger_info_skim.md` (CLOSED). | Dummy no longer needed. Build the correction in `mc_trigger_efficiency.md`; carry the ~20% MC over-efficiency of the **second** muon leg (L1 RoI × HLT) as a systematic — measured `f(2mu4\|mu4)` MC/data = 1.23 vs `f(mu4_mu4noL1\|mu4)` = 1.02. **MEASURED 2026-07-10 on the test samples** (`mc_trigger_efficiency.md`): pp ε_ΔR^2mu4 plateau 0.963 (≈1 ✓), small-ΔR suppression 0.77–0.84 below 0.2; overlay ε_ΔR^cross plateau 0.864 (flat systematic → **plateau-normalize before use**), small-ΔR enhancement ~2.2× rel. plateau. Remaining before application: plateau normalization + full-stat MC rerun |
...
| 2026-07-10 | **Trigger simulation ENABLED in Run-3 fullsim/overlay MC skims** (Q4 dR-correction prerequisite) | AMI (`doRDO_TRIG`+`doTRIGtoALL` on r16578/r17618/r17662) and the AODs both confirm trigger sim; `mu4`/`2mu4`/`mu4_mu4noL1` unprescaled. SkimCode gates trigger on a new `mc_has_trigger_sim` flag, not `is_MC` (`434de17`, `/review-analysis-code` PASS). **Caught `StoreAllEvents=false` silently trigger-OR-filtering the MC** (17% of events dropped, mu4→100%) — MC now keeps all events. Re-skim `July2026.v1`: 31/31 tasks, 10 000 entries each, 24/24 pp files bit-identical to backups outside trigger branches. MC/data `f(2mu4|mu4)`=1.23 vs `f(mu4_mu4noL1|mu4)`=1.02 ⇒ **MC over-estimates the SECOND muon leg (L1 RoI×HLT) efficiency by ~20%** → systematic on the MC ΔR correction. Tracking `mc_trigger_info_skim.md` (CLOSED) → `mc_trigger_efficiency.md`. |
| 2026-07-01 | **Template-fit correction ordering REVERSED → minv fit at RECO** | The low-mass minv template fit is done at RECONSTRUCTED level, after trigger-eff but BEFORE reco-eff/unfolding (reco-eff+unfolding correct the extracted N_S AFTER the fit; then acceptance). Physics: mixed-event uses real-data muons + fakes have no truth → both reco-only. Template-fit RDF fills (PP+PbPb `low_mass_template_calc`) switched to a TRIGGER-ONLY weight (`crossx_weight_trig_only`/`weight_for_dsigma_trig_only`); nominal crossx/R_AA unchanged. `/review-analysis-code`. S/G MC templates move truth→fullsim-reco (remaining). Tracking `low_mass_dimuon_template_fit.md`. |
