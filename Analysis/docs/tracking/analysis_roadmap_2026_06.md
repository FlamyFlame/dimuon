...
| 16 | Background subtraction (full program): (a) Δp/p fake-muon purity **[DEFERRED 2026-06-22 — future TODO needing π/K MC; preliminary chain treats purity as ~flat per Run 2 >98%]**, (b) correlated-physics (g→QQ̄ / FE) **minv template fit** with mixed-event combinatorial + **SS-anchored G normalization** (k=G_SS/G_OS), (c) signal acceptance | IN PROGRESS (autonomous impl. 2026-06-22) | Δp/p method from Run 2 note §DpopAna (deferred). The minv template-fit program is tracked in `low_mass_dimuon_template_fit.md` (autonomous implementation underway: ScrambGen mixed-event + acceptance + fitter; preliminary with placeholder reco-eff/unfolding). **NOTE: the current crossx/R_AA `OS − SS` subtraction (steps 14/15) is PROVISIONAL** — removes only combinatoric, leaving `(1−k)·G`; replaced by this program |
