...
| 2026-06-16 | Reco-eff PLACEHOLDER promoted to NOMINAL (user request; Run 3 MC ≥2-3 mo away) | `w_reco` folded into nominal corrected weight (`*_trig_corr` = base·w_reco·w_trig) → all crossx histos + R_AA 3D input now reco+trig corrected (== validated reco_trig stage; verified pp 5739, PbPb 3D ×1.67). `/review-analysis-code` PASS. Crossx reran + nominal crossx plots reran (pp24, pbpb_23_24_25_combined). Pre-reco backup `crossx_hist_backup_20260616_pre_reco_nominal/`. **R_AA plotting still BLOCKED** (`RAA_plotting.cxx` stale legacy: Mac paths, old filenames, non-produced SS histo → task_06); reco is in the R_AA input only. Crossx/R_AA remain placeholder/preliminary |
