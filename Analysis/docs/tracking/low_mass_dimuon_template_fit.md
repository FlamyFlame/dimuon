...
NEXT: method 6 — signal-region (pair_pt>8, q·η<2.2) data/MC comparison with `_no_res_cut` data; **tight-WP fake/hadronic study UNDERWAY** (user 2026-06-30, background subagent `_sub_tight_wp_1.md`: add useTight to fill_weighted_fullsim.C + bkg_mc_provenance.C, regenerate plots, quantify medium-vs-tight fake/hadronic for the Δp/p strategy). Then resume BUILD (OS−SS + S+G fitter → acceptance → R_AA).
