...
- **Overlay**: Two classes of B-meson chains exist. (1) **Pythia signal B-mesons** (low bc, e.g. 206, 264): the full ancestry is present — b-quark appears among the B-meson's multiple parents, and `FindHeavyQuarks` finds it normally. These chains are NOT truncated. (2) **Geant4 secondary B-mesons** (bc 3939–73710, measured Apr 2026): B-meson entries created by Geant4/EVTGEN during detector simulation, assigned sequential barcodes in the Pythia-range (bc < 200,000). Note: HIJING particles have bc > 200,000 and are entirely distinct — these are not HIJING particles. Geant4 stores their parent pointers as dummy beam-particle entries [proton bc=1, gluon bc=2] because it does not record the QCD production chain. The loop exits when `parent_ids[0]` is the proton → stores `bc=1` for **all** such chains → `m1 == m2 == 1` for all → `from_same_b=true` for all, which is wrong.

  PP fullsim has 0 of these: the smaller event record means Geant4 B-meson barcodes tend to collide with the original Pythia B-meson barcode and get deduplicated by the Bug 2 fix. In overlay, the large particle count (~78k) gives Geant4 B-mesons genuinely new barcodes, so the Geant4 entry (with truncated parents) is what gets looked up.
...
For Geant4 secondary B-meson chains, `FindHeavyQuarks` returns −1 (b-quark absent; only beam-particle dummy parents stored) → `skip_event_origin_analysis = true`. Measured on 500-event kn=0 test (Apr 2026): **2.2% of B-hadron muons** in overlay hit this path (84/3789); eldest B-hadron barcodes span 3939–73710 (all < 200,000 — Geant4-range, not HIJING which is > 200,000). PP fullsim has 0 truncated chains.

The guard `if (abs(first_hadron_id) == 5)` prevents calling `PrintHistory` for these chains — `first_hadron_id` here is the ID of the particle **above** the eldest B-hadron (the particle that caused the B-hadron tracing loop to exit), not the B-hadron itself. For normal chains it is 5 (b-quark); for Geant4 secondary chains it is 2212 (proton). Calling `PrintHistory` on the proton chain would crash: gluon bc=2 has no valid parent in `truth_barcode`.

The warning print "Previous HQ barcode is -1" is suppressed when `abs(first_hadron_id) != 5` (Geant4 secondary case, added in `dee4050`).
