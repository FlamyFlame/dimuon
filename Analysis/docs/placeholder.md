...
Last updated: 2026-09-18. (Item 3: pp ε_reco now contains the gap acceptance, R_AA mixed-fiducial;
item 7: the pp24 hybrid trigger weight. See `docs/tracking/intnote_trig_reco_eff_update_2026_09.md`
and `docs/tracking/pair_reco_eff_gap_acceptance.md` for the code evidence.)
...
| 3 | **Reconstruction efficiency** — Run 2 single-muon ε_reco proxy (ε₁·ε₂), standing in for the proper 3D pair ε_reco(pair pT, pair η, dR). **pp24 IS NO LONGER A PLACEHOLDER (2026-08-18):** it uses the measured pp24-fullsim 3D pair efficiency (`pair_reco_eff_pp24_full.root`; since 2026-09-17 the gap cuts sit on the RECO leg only so ε_reco CONTAINS the gap acceptance, no ε_acc: inclusive 0.5850 Tight / 0.6360 Medium) via `Utilities/PairRecoEffEvaluator.h`. PbPb only — and because the PbPb proxy carries NO gap acceptance while pp now does, **R_AA is mixed-fiducial and must not be quoted** (`pair_reco_eff_gap_acceptance.md` R3). | PbPb (all yrs) | **Applied** in crossx as a correction STAGE (`_corr_unfolded_reco[_trig]` hists); **PbPb = colleague's EXACT Run 2 single-μ TF1 fits, BOTH WPs** (`MuonRecoEffcyRun2MC_{tight,medium}.root`, WP-matched key, nominal Tight; evaluated at exact pT); **pp is NOT a placeholder** (measured pp24-fullsim 3D pair eps_reco since 2026-08-18) | Full Pythia fullsim HIJING-overlay (r17662) + full pp24 fullsim → proper 3D pair ε_reco | `RDFBasedHistFilling{Data,PP,PbPb}` (`EvaluateSingleMuonRecoEffPlaceholder`, `CorrectionStages.h`); `EfficiencyCorrs/EffFiles/run2_reco_eff_placeholder.root` | Q4, ledger 12; tracking `reco_eff_placeholder_run2.md` |
...
| 6 | **PbPb 2026 lumi + GRL** — registered 2026-09-10 (`IntNotes/data/luminosity/pbpb_2026/lumitable_pbpb_26_HLT_mu4.csv`, GRL total 2.62316 nb⁻¹, `physics_HI2026_50ns_noIBL.xml`; `pbpb2026_analysis_support.md`). **Still preliminary:** two 2026 runs (522384, 522408; 6.5 % of the 2026 lumi) are under expert review for an anomalous track multiplicity, and `IntNotes/tex/datasets.tex` does not yet describe 2026 (the note carries a \PHtext where 2026 appears). | PbPb 2026 | full-GRL lumi, no bad-run list; 2026 turn-ons and crossx produced | expert decision on the two runs; datasets chapter update | `IntNotes/data/luminosity/pbpb_2026/`, `PbPbSampledLumi.h` | Q2.1/Q2.5 |
| 7 | **dR trigger-correlation correction** — **pp24 IS NO LONGER A DUMMY (2026-08-18):** the pp24 2mu4 pair weight is a HYBRID (`Utilities/PairTrigEffCrossxEvaluator.h`, 2026-09-17, `pp24_trig_eff_hybrid_application.md`): below 74.24 GeV data ε₁ε₂ × MC ε_dR on 6 pair-pT bins × 3 |η^pair| groups (`nocorr_etamerge`, opposite sign, expo primary / polynomial primary in 3 named forward cells / interp fallback, no raw tier; inclusive OS 0.78 at dR→0), above 74.24 GeV the single-value MC pair efficiency (signal window) × SF₁SF₂; TEMPORARY: one refused cell served by the pT-merged cell (D1), SS pairs on OS numbers (D2); no closure yet on these inputs. **PbPb still applies the BARE union with NO ε_dR at all** (`RDFBasedHistFillingPbPb.cxx:1060`) — the dR-corrected union in `mc_trigger_efficiency.md` §2 is documented but not implemented. | PbPb | dummy ε_dR ≡ 1; **and the former ΔR>0.05 SIGNAL cut was removed 2026-06-22 (interim nominal)** — both pending the MC-based dR trigger correction that decides whether any ΔR cut is needed | Fullsim overlay with trigger sim | crossx pipeline (dR corr = 1; no ΔR signal cut); `docs/tracking/remove_dr_cut_signal_selection.md` | Q4 |
...
- the colleague's **exact Run 2 fits** from the Run 2 dimuon analysis (ATL-COM-PHYS-2021-1094) —
  ...
  (`RDFBasedHistFillingData.cxx:813`); the muon pT is clamped to [4, 19] GeV at the point of use.
- Written into `EfficiencyCorrs/EffFiles/run2_reco_eff_placeholder.root` by
  ...
  `RDFBasedHistFillingPbPb.cxx:1062-1075` as ε₁·ε₂, floored at 0.05 before inversion.
