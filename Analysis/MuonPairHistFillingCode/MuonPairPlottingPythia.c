    ...
    for (int ifile = 0; ifile < nFiles; ifile++){
        ...
        for (int jkin = 0; jkin < nKinRanges; jkin++){
            for (int ksign = 0; ksign < ParamsSet::nSigns; ksign++){
                ...
                inTree[ifile][jkin][ksign]->SetBranchStatus("from_same_resonance"           ,1);
...
void MuonPairPlottingPythia::FillHistograms(int nfile, int nkin, int nsign){
    ...
    // ------------------------------------------------------------------------------------------------------------------------
    int ancestor_ind = nAncestorGroupsTotal; // other origin categories

    if (from_same_resonance[nfile][nkin][nsign]){
        ancestor_ind = nAncestorGroups;
    }else if (from_same_b[nfile][nkin][nsign]){
        ancestor_ind = nAncestorGroups + 1;
    }else{ // the "proper ancestor groups" for only for pairs both from (different) open HF's - no overlap with resonance or single-b
    ...
    h_minv_pair_pt_jacobian_corrected_ancestor_binned[nsign][ancestor_ind]->Fill(pair_pt[nfile][nkin][nsign],minv[nfile][nkin][nsign],ev_weight * 1. / minv[nfile][nkin][nsign]);
    h_minv_pair_pt_zoomin_jacobian_corrected_ancestor_binned[nsign][ancestor_ind]->Fill(pair_pt[nfile][nkin][nsign],minv[nfile][nkin][nsign],ev_weight * 1. / minv[nfile][nkin][nsign]);
