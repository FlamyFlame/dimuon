...
private:
    std::string data_dir = "/Users/yuhanguo/Documents/physics/heavy-ion/dimuon/datasets/pbpb_2023/";
...
private:
    ...
    void drawOneCanvas(std::string trg, std::string musign, std::string ctr) // draw one canvas for one (trigger, muon sign) pair
    {
        if (debug_mode) std::cout << "Calling function drawOneCanvas for trigger " << trg << ", muon sign " << musign_maps[musign] << std::endl;
        gStyle->SetOptStat(0);

        TCanvas* c = new TCanvas(Form("c_%s%s_%s", trg.c_str(), ctr.c_str(), musign.c_str()), "Trigger Turn-on Curves", 1500, 1000);
        c->Divide(4,3);
        c->SetGrid();
        ...
        for (auto q_eta_bin : q_eta_bins_for_pT_trg_effcy_graphs){
            ...
            std::string gname = "g_pt2nd_vs_q_eta2nd";
            gname += ctr + "_" + musign + "_" + trg_maps[trg] + "_sepr_py_" + q_eta_bin + "_divided";
        ...
        c->SaveAs(Form("%s%strg_effcy_pT_fitting_%s%s_%s.png", data_dir.c_str(), fitting_outdir.c_str(), trg_maps.at(trg).c_str(), ctr.c_str(), musign_maps.at(musign).c_str()));
...
void single_muon_trig_effcy_pT_fitting_PbPb (){
    ...
    fitter->fitting_mode = SingleMuEffcyPtTurnOnFitter::fermi_plus_log;
    fitter->Run();
}
