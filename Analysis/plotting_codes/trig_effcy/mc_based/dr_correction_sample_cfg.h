...
inline DrCorrSample GetDrCorrSample(const std::string& key)
{
    DrCorrSample s;
    s.key = key;
    if (key == "pp") {
...
// Medium-WP products go into a medium/ SUBDIRECTORY of each plot dir; Tight stays in its root.
inline std::string DrCorrWpDir(bool use_tight_wp) { return use_tight_wp ? "" : "medium/"; }
