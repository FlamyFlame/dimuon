...
std::vector<std::vector<std::string>> hist_names = {{"h_QQ_both_from_Q_ancestor_sp_sign1_near","h_QQ_both_from_Q_ancestor_sp_sign1_away"},{"h_QQ_both_from_Q_ancestor_sp_sign2_near","h_QQ_both_from_Q_ancestor_sp_sign2_away"}};
...
void mc_muon_pair_sp_ancestor_groups(){
  ...
  for (int imc = 0; imc < nMCmodes; imc++){
    ...
    c->SaveAs(Form("/usatlas/u/yuhanguo/usatlasdata/dimuon_data/plots/powheg/prt_grouping/muon_pair_sp_ancestor_groups%s.png",mcmodes[imc].c_str()));
    c->Close();
    delete c;
  }
}
