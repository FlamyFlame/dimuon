...
void PlotMCDataSingle2DHistogram::Run(){
    ...
    for (unsigned int idt = 0; idt < s_nDtTypes ; idt++){
        ...
        if (draw_2d_hist){
            ...
            c->SaveAs(Form("/usatlas/u/yuhanguo/workarea/dimuon_codes/plots/mc_data_compr/hist_2D/%s.png", kin.c_str()));
            c->Close();
            delete c;
        }
        ...
        if (!proj_intervals.empty() && idt != DataType::powheg_bb) { // powheg bb already added to cc
            if (intvls_same_canvas){
                ...
                if (kiny != "" && kinx != "")   proj_filename = "/usatlas/u/yuhanguo/workarea/dimuon_codes/plots/mc_data_compr/hist_2D_interval_projections/" + kiny + dt_suffix[idt] + "_" + kinx + "_intvls" + ".png";
                else              proj_filename = "/usatlas/u/yuhanguo/workarea/dimuon_codes/plots/mc_data_compr/hist_2D_interval_projections/" + kin + "_projY_" + dt_suffix[idt] + "_intvls" + ".png";
                ...
                for (size_t intvl = 0; intvl < proj_intervals.size(); ++intvl) {
                    ...
                    std::string proj_filename;
                    if (kiny != "" && kinx != "")   proj_filename = "/usatlas/u/yuhanguo/workarea/dimuon_codes/plots/mc_data_compr/hist_2D_interval_projections/" + kiny + dt_suffix[idt] + "_" + kinx + "_intvl" + std::to_string(intvl) + ".png";
                    else              proj_filename = "/usatlas/u/yuhanguo/workarea/dimuon_codes/plots/mc_data_compr/hist_2D_interval_projections/" + kin + "_projY_" + dt_suffix[idt] + "_intvl" + std::to_string(binX1) + "-" + std::to_string(binX2) + ".png";
                    projCanvas->SaveAs(proj_filename.c_str());
