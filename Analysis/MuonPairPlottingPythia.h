    ...
    if (mode == 1){
        for (unsigned int ksign = 0; ksign < ParamsSet::nSigns; ksign++){
            ...
            for (int kflav = 0; kflav < nFlavors; kflav++){
                ...
                h_DR_zoomin_flavor_binned[ksign][kflav] = new TH1D(Form("h_DR_zoomin_sign%d%s",ksign+1,flavor_grps[kflav].c_str()),";#Delta R;1/N_{evt} dN/d#Delta R", nDR_zoomin_bins,0,pms.deltaR_thrsh_zoomin);
                h_DR_jacobian_corrected_flavor_binned[ksign][kflav] = new TH1D(Form("h_DR_jacobian_corrected_sign%d%s",ksign+1,flavor_grps[kflav].c_str()),";#Delta R;#frac{1}{N_{evt}} #frac{1}{#Delta R} #frac{dN}{d#Delta R}", nDR_bins,0,pms.deltaR_thrsh[2]);
                h_DR_zoomin_jacobian_corrected_flavor_binned[ksign][kflav] = new TH1D(Form("h_DR_zoomin_jacobian_corrected_sign%d%s",ksign+1,flavor_grps[kflav].c_str()),";#Delta R;#frac{1}{N_{evt}} #frac{1}{#Delta R} #frac{dN}{d#Delta R}", nDR_zoomin_bins,0,pms.deltaR_thrsh_zoomin);
