...
cd "${SCRIPT_DIR}"
echo "[RUN] pbpb23 mixed-event template (T_mix) pass (trigger_mode=1, _scrambled, mixed-event)..."
root -l -b <<ROOTEOF
...
pbpb.Run();
std::cout << "\\n[RUN] pbpb23 mixed-event template completed successfully!" << std::endl;
.q
ROOTEOF
echo "[RUN] pbpb23 mixed-event template completed. Output file:"
