...
# Reads the *_scrambled ntuples (ScrambGen mixed-event) (resonances PRESENT) and writes the 0-4 GeV minv
...
echo "[RUN] pp24 mixed-event template (T_mix) pass (trigger_mode=3, _scrambled, mixed-event)..."

root -l -b <<EOF
.L RDFBasedHistFillingPP.cxx+
RDFBasedHistFillingPP pp(24);
pp.trigger_mode = 3;              // 2mu4 (same as nominal crossx)
pp.low_mass_template_calc = true;
  pp.mixed_event_template = true; // -> _scrambled input + _template_fit_mixed_event output; minv-only fill
pp.output_generic_hists = false;  // light pass: weights computed inline in the template block
pp.output_gapcut_hists  = false;
std::cout << "\\n[RUN] Starting pp24 mixed-event template-fit run..." << std::endl;
pp.Run();
std::cout << "\\n[RUN] pp24 mixed-event template-fit completed successfully!" << std::endl;
gSystem->Exit(0);
EOF

echo "[RUN] pp24 mixed-event template-fit completed. Output file:"
