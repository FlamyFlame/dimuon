	...
	if (run_year == 23 || run_year == 24 || run_year == 25 || run_year == 26){
        ...
        if (low_mass_template_calc) { // low-mass template-fit pass
            ...
            const std::string tmpl_path = mixed_event_template
                ? base + "_scrambled.root"
                : base + "_no_res_cut.root";
            if (gSystem->AccessPathName(tmpl_path.c_str())) {
                throw std::runtime_error(
                    "RDFBasedHistFillingPbPb: low-mass template-fit input not found: " + tmpl_path +
                    " (run_year=20" + run_year_str + "). " +
                    (mixed_event_template ? "mixed_event_template requires the *_scrambled.root (ScrambGen output)."
                                          : "the template-fit pass requires the _no_res_cut ntuples."));
            }
            in_path = tmpl_path;
...
void RDFBasedHistFillingPbPb::FillTrigEffcyHistsInvWeightedbySingleMuonEffcies() {
    ...
    auto fillInvWeightedHists = [&](ROOT::RDF::RNode node, const std::string& pair_sign, const std::string& cat_suffix) {
        // Cross-term: P(both muons fire mu4), inverse-weighted by 1/(ε₁·ε₂).
        // NOTE: on a mu4-selected sample this ratio is biased high by 1/P(at least one fires),
        // because the denominator only contains triggered events. Kept as reference for
        // unbiased MC samples where no trigger selection exists.
...
void RDFBasedHistFillingPbPb::OpenEffcyPtFitFile() {
    ...
    std::string hist_path = base_dir + "/histograms_real_pairs_pbpb_20" + yr + "_single_mu4_coarse_q_eta_bin_qeta_fid" + wpsuf + ".root";
...
void RDFBasedHistFillingPbPb::FillHistogramsCrossx(){
    ...
    ROOT::RDF::RNode df_with_trig = df_single_b_crossx
        ...
        // (eps1*eps2 proxy), per-centrality via avg_centrality. Floor the pair eff
        // at 0.05 to bound the 1/eps blow-up near threshold; lookup failure -> w_reco=1.
        // ctr < 0 (invalid PbPb centrality) must NOT fall through to the pp
        // barrel/endcap graphs (centrality<0 is the pp sentinel); return 1 (no
        // reco correction) instead.
    ...
    // every crossx histogram filled with weight_for_RAA_trig_corr (incl. the
    // R_AA 3D input h3d_op_..._vs_centr...) is reco+trig corrected. Equals the
    // _corr_unfolded_reco_trig stage by construction (w_unfold==1). To revert to
    // trig-only, drop the "w_reco *" factor. (docs/tracking/reco_eff_placeholder_run2.md)
    ...
    for (int ictr = 0; ictr < nCtrBins; ++ictr){
        ...
        // primary pair_pt x pair_eta differential, per centrality. See CorrectionStages.h.
